...
//Function returns an image
    ...
    private static function code128BarCode($code, $density = 1)
    {
    ...
    public static function generarBarcode($text, $space = 1)
    {
        $img = static::code128BarCode($text, $space);
        ob_start();
        imagepng($img);
        $output_img = ob_get_clean();

        return "<div style='text-align: center;width:340px;font-size:13px'><img width='100%' src='data:image/png;base64," . base64_encode($output_img) . "' alt='{$text}' /><br/>{$text}</div>";
    }
}
