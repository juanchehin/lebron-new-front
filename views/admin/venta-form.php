<div class="row">
    ...
    <?php
        ...
        $query = Usuario::orderBy("id_usuario", "DESC");

        $usuarios = $query->where("borrado",'0')
                        ->where("activo",'1')
                        ->get();  
        ...
        foreach ($usuarios as $item) {
            $usuarios_select[] = $item->usuario;
        }
    ?>

    <div class="col-md-8" id="dv-venta-form-group">
        <div class="panel panel-info">
            <div class="panel-body">
                <div class="row">
                    <div class="form-group col-md-3">
                        <label for="local">Local <i class="required"></i></label>
                        <!--  -->
                        <?php 
                            $request_uri = $_SERVER['REQUEST_URI'];
                            
                            $palabra_clave = "stock/ingreso";
                            
                            $posicion = strpos($request_uri, $palabra_clave);

                        if ( $posicion !== false): ?>
                            <input type="hidden" id="local" name="id_local" value="6" disabled="disabled"/>
                            <h4 style="margin: 2px 0 0; border: 1px solid #ccc;padding:3px 5px">Negocio Mitre</h4>
                        <?php else : ?>
                            <!-- Usuario 46 accede a local 25 y mitre -->
                            <?php if (($admin_user->id_usuario == 46) ): ?>
                                <select id='local' name='id_local' class='form-control'>
                                    <option value=''>Local</option>
                                    <option value='12'>Local 25 de mayo</option>
                                    <option value='6'>Negocio Mitre</option>
                                </select>
                            <?php else : ?>                            
                                <?php if ( !$local_id = $admin_user->id_local ): ?>
                                    <?= $selectLocal ?>
                                <?php else : ?>
                                    <input type="hidden" id="local" name="id_local" value="<?= $local_id ?>"/>
                                    <h4 style="margin: 2px 0 0; border: 1px solid #ccc;padding:3px 5px"><?= $admin_user->local ?></h4>
                                <?php endif; ?>
                            <?php endif; ?>  
                        <?php endif; ?>                      
                    </div>
                    <div class="form-group col-md-3" id="dv-local-destino"></div>
                    <div class="form-group col-md-3" id="dv-nro-factura">
                        <label for="factura">N&deg; Comprobante</label>
                        <?php if ( false ): ?>
                            <div class="input-group-addon" style="width:.35%">
                                <select name="letra" id="letra" class="form-control">
                                    <?php foreach (["A", "B", "C"] as $letra): ?>
                                        <option><?= $letra ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        <?php endif; ?>
                        <input type="tel" id="factura" name="factura" class="form-control" value="<?= $venta->factura ?>" maxlength="15">
                    </div>
                   
                    <div class="form-group col-md-3">
                        <label for="fecha">Fecha:</label>

                        <?php if (!( $admin_user->es_admin || ($admin_user->id_usuario == 11) || ($admin_user->id_usuario == 21) || ($admin_user->id_usuario == 16) || ($admin_user->id_usuario == 25) || ($admin_user->id_usuario == 23) || ($admin_user->id_usuario == 37) || ($admin_user->id_usuario == 38) || ($admin_user->id_usuario == 47) || ($admin_user->id_usuario == 48) || ($admin_user->id_usuario == 30) || ($admin_user->id_usuario == 43))): ?>

                            <!-- Fecha hoy -->
                            <input type="text" readonly="readonly" autocomplete="off" class="form-control" name="fecha-hoy" id="fecha-hoy" value="<?php echo date('d/m/Y'); ?>" />

                        <?php else : ?>
                            <!--  -->
                            <?php if ( ($admin_user->id_usuario == 37)): ?>

                                <!-- Hasta 2 dias atras -->
                                <input type="text" autocomplete="off" class="form-control" name="fecha" id="fecha-user16" value="<?= $venta->fecha ?>">

                            <?php else : ?>

                                <?php if ( ($admin_user->id_usuario == 43) || ($admin_user->id_usuario == 38) || ($admin_user->id_usuario == 47) || ($admin_user->id_usuario == 30) || ($admin_user->id_usuario == 48)): ?>

                                    <?php if (($admin_user->id_usuario == 48) || ($admin_user->id_usuario == 30)): ?>
                                        <!-- Hasta 1 mes dias atras -->
                                        <input type="text" autocomplete="off" class="form-control" name="fecha" id="fecha-user48" value="<?= $venta->fecha ?>">
                                    <?php else : ?>
                                        <!-- Hasta 5 dias atras -->
                                        <input type="text" autocomplete="off" class="form-control" name="fecha" id="fecha-user38" value="<?= $venta->fecha ?>">

                                    <?php endif; ?>

                                <?php else : ?>

                                        <input type="text" autocomplete="off" class="form-control" name="fecha" id="fecha" value="<?= $venta->fecha ?>">
                ...
                <!--  -->
                <form id="frm-venta" action="!AdminVenta/guardarVenta">
                    <div class="row">
                        ...
                        <div class="form-group col-md-2">
                            <?php if ( $admin_user->id_usuario == 38 ): ?>
                                <!--  -->
                                <?php if ( $operacion == MenuPanel::menuVentas ): ?>
                                    ...
                                    <select name="tipo_venta" id="tipo-venta" class="form-control" required>
                                        <option value="">Seleccionar</option>
                                            <option class="option-tipo-venta" value="presupuesto publico">Presupuesto</option>
                                            <option class="option-tipo-venta" value="presupuesto mayorista">Presupuesto mayorista</option>
                                            <option class="option-tipo-venta" value="publico">Publico</option>
                                    </select>
                                <?php else : ?>
                                    <h4>&nbsp;</h4>
                                <?php endif; ?>
                        ...
                        <!-- ***** Medio de pago  ******* -->
                        <div class="form-group col-md-4" id="dv-pago-control">
                            <?php if ( $clientes ): ?>
                        ...
                        <!-- ***** Estado  ******* -->
                     <div class="form-group col-md-3" id="estado-venta">
                        <label for="">Estado :</label>
                            <div class="input-group-addon" style="width:.35%">
                                <select name="estado-venta" id="estado-venta" class="form-control">
                                        <option value="r" selected="selected">Realizada</option>
                    ...
                    <!--  -->
                        ...
                        <div class="form-group col-md-4">
                            <label for="">
                                    Nro. Tarjeta
                                    <input class="form-control form-file" type="text" name="nro_tarjeta_input" size="20" id="nro_tarjeta_input" disabled>
                            </label>
                        </div>
        ...
        <button type="button" id="btn-cerrar-vta" class="pull-right btn btn-warning">Cerrar Operaciones</button>
        <button type="button" id="btn-refrescar-variables" class="pull-right btn btn-warning">Refrescar variables</button>

    </div>
    <div class="col-md-4">
        <table class="data-table" id="tbl-linea-venta">
            <thead>
            <tr>
...
<script type="text/javascript">
    ...
    const fechaUser38 = document.getElementById('fecha-user38');
    const fechaUser48 = document.getElementById('fecha-user48');
  
    var boton = document.getElementById('btn-refrescar-variables');
    boton.addEventListener('click', refrescar_variables);

    // ========================================
    // 
    // ========================================
    function refrescar_variables()
    {
        precio_arriba = document.getElementById('h-total-span');
        suma_precio_productos = get_total().toFixed(2); 
        precio_arriba.innerText = suma_precio_productos; 
        precio_abajo = document.getElementById('pc-spn-total');
        precio_abajo.innerText = suma_precio_productos;
        pc_pagos = {};

        var linea_pago = document.querySelector('.linea-pago');
        linea_pago.setAttribute("disabled", false);
    ...
    function opcion_pago(id_cuenta, importe)
    {
        ...
        // usuario 38 solo puede acceder al medio 'mercado libre'
        <?php if ($admin_user->id_usuario == 38): ?>

            line += `<option color:white;" value='43'>Mercado Libre</option>`;
        ...
        nro_tarjeta_input = document.getElementById('nro_tarjeta_input');
        ...
        cuentaSelect.onchange = function () {
            ...
            if ( cuentaSelect.value == 36 )
            {
                nro_tarjeta_input.disabled = false;
            }else{
                nro_tarjeta_input.disabled = true;
            }
        ...
        if ( (btn = document.getElementById(`btn-${id}`)) )
        {
            ...
            btn.onclick = function () {
                ...
                console.log('cuentaSelect.value::: ', cuentaSelect.value);
                ...
                if ( !pc_pagos[id + "&" + cuenta] )
                {
                    ...
                    console.log('in_monto::: ', in_monto);

                
                // ****************** Calculo tarjetas cuotas *****************

                console.log('cuentaSelect.value::: ', cuentaSelect.value);
                // ¿Es tarjeta 1/3/6 pagos ?
                switch (cuentaSelect.value) {
                    case "40":    // 1 pago
                        console.log('40::: ');

                        // calculo el monto a sumar
                        porcentaje_aumento = 10;
                        aumento_lineal = ( (porcentaje_aumento * in_monto )/ 100);
                        // sumo al monto final el aumento lineal
                        total_pago = total_pago + aumento_lineal;                       
                        // sumo el aumento al valor del item de tipo pago
                        in_monto = in_monto * (1 + porcentaje_aumento / 100);

                        // labelMontoCompra = labelMontoCompra * (1 + porcentaje_aumento / 100);

                        // modifico el total de la compra
                        labelTotal = document.getElementById('h-total');
                        labelTotal.querySelector('span').innerText = total_pago;

                        // precio_abajo = document.getElementById('pc-spn-total');
                        // precio_abajo.innerText = suma_precio_productos;

                        // Modifico el resto de la compra (precio abajo)

                        // opcion_pago();

                        // total_pago

                        // console.log('aumento_1::: ', aumento_1);

                        // total_pago = total_pago + aumento_1;
                        // in_monto = in_monto + aumento_1;
                        // monto = monto + aumento_1;
                        // labelMontoCompra.innerText = labelMontoCompra.innerText + aumento_1;
                        break;
                    case "41":    // 3 pago
                        porcentaje_aumento = 15;
                        let aumento_3 = total_pago * (1 + porcentaje_aumento / 100);
                        total_pago = total_pago + aumento_3;
                        in_monto = in_monto + aumento_3;
                        monto = monto + aumento_3;
                        labelMontoCompra.innerText = labelMontoCompra.innerText + aumento_3;
                        break;
                    case "42":    // 6 pago
                        porcentaje_aumento = 20;
                        let aumento_6 = total_pago * (1 + porcentaje_aumento / 100);
                        total_pago = total_pago + aumento_6;
                        in_monto = in_monto + aumento_6;
                        monto = monto + aumento_6;
                        labelMontoCompra.innerText = labelMontoCompra.innerText + aumento_6;
                        break;
                    default:
                        break;
                    }

                // ****************** Fin calculo tarjetas cuotas *****************
                    ...
                    console.log('montoInput.value::: ', montoInput.value);
    ...
    else
    {
        ...
        else
        {
            if(fechaUser38 !== null)
            {
                inputfechaUser16 = $('#fecha-user38');
            }
            else
            {
                if(fechaUser48 !== null)
                {
                    inputfechaUser48 = $('#fecha-user48');
                }
                else{
    ...
    selectLocal.onchange = function () {
        ...
        if ( id && id !== attrs["id_local"] )
        {
            fetch("!AdminMain/setLocal", {
                "method": "POST",
        ...
        es_online = (id === parseInt(<?=intval($localML)?>));
        if ( tipoVentaSelect )
        {
            // tipoVentaSelect.onchange();
        }
        selectDestinoHtml = "";
        if ( id && operacionTraspaso )
        {
            localesDestino = <?=json_encode(Local::$_LOCALES)?>;
            selectDestinoHtml += `<label for="destino">Destino <i class="required"></i></label>`;
            selectDestinoHtml += `<select name="destino" id="destino" class="form-control">`;
            selectDestinoHtml += `<option>Seleccionar</option>`;
            for (lcl in localesDestino)
            {
                if((lcl != 9) && (lcl != 11) && (lcl != 13) && (lcl != 8))
                {                    

                    if ( (parseInt(lcl) === id) || (lcl === "<?=$localML?>") )
                    {
                        continue;
                    }
                    selectDestinoHtml += `<option value="${lcl}">${localesDestino[lcl]}</option>`;
                }
            }
            selectDestinoHtml += `</select>`;
        }
        document.getElementById('dv-local-destino').innerHTML = selectDestinoHtml;
        dvLineasPago.style["min-height"] = "115px";
        //--
        if ( (!id || attrs["block"]) && (fechaBanderaAdmin !== null))
        {
            if ( !id )
            {
                inputFecha[0].setAttribute("disabled", true);
            }
            stop.innerHTML = "<div style='position: absolute;z-index:6;left:0;right:0;height:100%;width:100%;background:transparent'></div>";
        }
    };
    ...
    else
    {
        ...
        else
        {
            ...
            if(fechaUser38 !== null)
            {
                console.log("pasa fechaUser38")
                var cincoDiasAtras = new Date();
                cincoDiasAtras.setDate(cincoDiasAtras.getDate() - 14);
                ...
                inputfechaUser16.click(function () {
                    document.getElementById('ui-datepicker-div').style["z-index"] = 7;
                        ...
                        attrs["fecha"] = this.value;
                        historial_articulos();
                }).css("text-align", "right").calendario({"defaultDate": new Date(), "maxDate": new Date(),minDate: new Date(cincoDiasAtras),"yearRange": "-1:+0"});

                // inputfechaUser38[0].value = (attrs["fecha"] || "");
            }
            else
            {
                if(fechaUser48 !== null)
                {
                    ...
                    inputfechaUser48.click(function () {
                        document.getElementById('ui-datepicker-div').style["z-index"] = 7;
                        }).on("change", function () {
                            attrs["fecha"] = this.value;
                            historial_articulos();
                    }).css("text-align", "right").calendario({"defaultDate": new Date(), "maxDate": new Date(),minDate: new Date(unMesAtras),"yearRange": "-1:+0"});
    ...
    // Cerrar venta
    // ========================================
    document.getElementById('btn-cerrar-vta').onclick = function () {
        if ( attrs["block"] || !attrs["fecha"] )
        {
            return;
        }
        jconfirm(function (cnf) {
            if ( !cnf )
            {
                return;
            }
            fetch(`!AdminVenta/setEstado?fch=${attrs["fecha"]}`);
            attrs["block"] = 1;
            selectLocal.onchange();
        }, `Finalizar registro de operaciones para el día ${attrs["fecha"]}`);
    };
    ...
    // ========================================
    // Eliminar una linea venta (un producto)
    // ========================================
    function eliminar(id)
    {
        jconfirm(function (st) {
            if ( st )
            {
                fetch(`!AdminVenta/eliminarLinea?id=${id}`);
    ...
    function get_total(stg = false)
    {
        ...
        dvPagoControl = document.getElementById('dv-pago-control');
        dvPagoControl.removeAttribute("style");

        console.log('attrs["tipo_venta"]::: ', attrs["tipo_venta"]);
        if ( (esPresupuesto = (attrs["tipo_venta"] === "<?=strtolower(Venta::presupuesto)?>")) ||  (esPresupuesto = (attrs["tipo_venta"] === "<?=strtolower(Venta::presupuesto_mayorista)?>")) )
        {
            dvPagoControl.style["display"] = "none";
        ...
        if ( typeof labelMontoCompra !== "undefined" )
        {
            total_operacion = Boolean(<?=$venta->es_presupuesto?>) ? 0 : parseFloat(<?=floatval($total_op)?>);
            labelMontoCompra.innerText = (total - total_operacion);
            if ( !total || total !== total_operacion )
            {
                dvLineasPago.innerHTML = "";
                pc_pagos = {};
                if ( !esPresupuesto )
    ...
    // ========================================
    // Genera el listado de ventas en el recuadro de la
    // derecha
    // ========================================
    ...
    function dt_delete(opt)
    {
                jconfirm(function (res) {
...

                    {
                        ...
                        $.post('!AdminVenta/eliminar', {'id': opt}, function () {
    ...
    if ( (frmVenta = document.getElementById('frm-venta')) )
    {
        ...
        function finalizar_venta()
        {
            ...
            if ( !operacionTraspaso )
            {
                ...
                if ( ((attrs["tipo_venta"] !== "presupuesto") && (attrs["tipo_venta"] !== "presupuesto publico") && (attrs["tipo_venta"] !== "presupuesto mayorista")) && parseFloat(labelMontoCompra.innerText) )
                {
                    jdialog("Ingresar medio/s de pago y monto (Pago)");
                    return;
                }
            }
            //return;
            ...
            var rutaSinDominio = window.location.pathname + window.location.search;

            // obtener usuarios
            if (rutaSinDominio.indexOf("traspaso") !== -1) {

             

                let msg2 = `Usuario que confirma el traspaso :  
                                <select class="form-control">
                                    <?php foreach ($usuarios_select as $option): ?>
                ...
                // 
                let msg = `Esta apunto de realizar una venta de tipo :  <p style="background-color:tomato; font-size:160%; text-align:center;">` + attrs["tipo_venta"].toUpperCase() + `</p>¿Desea continuar?`;
